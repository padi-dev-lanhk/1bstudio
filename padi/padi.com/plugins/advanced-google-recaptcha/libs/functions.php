...
class WPCaptcha_Functions extends WPCaptcha
{
    ...
    static function handle_captcha()
    {
        ...
        if ($options['captcha'] == 'recaptchav2') {
            if (!isset($_POST['g-recaptcha-response']) || empty($_POST['g-recaptcha-response'])) {
                return new WP_Error('wpcaptcha_recaptchav2_not_submitted', __("<strong>ERROR</strong>: reCAPTHCA verification failed.<br /><br />Please try again.", 'advanced-google-recaptcha'));
            } else {
                ...
                if ($response->success) {
                    ...
                    return new WP_Error('wpcaptcha_recaptchav2_failed', __("<strong>ERROR</strong>: reCAPTHCA verification failed.<br /><br />Please try again.", 'advanced-google-recaptcha'));
                }
            }
        } else if ($options['captcha'] == 'recaptchav3') {
            if (!isset($_POST['g-recaptcha-response']) || empty($_POST['g-recaptcha-response'])) {
                return new WP_Error('wpcaptcha_recaptchav3_not_submitted', __("<strong>ERROR</strong>: reCAPTHCA verification failed.<br /><br />Please try again.", 'advanced-google-recaptcha'));
            } else {
                $secret = $options['captcha_secret_key'];
                $response = wp_remote_get('https://www.google.com/recaptcha/api/siteverify?secret=' . $secret . '&response=' . $_POST['g-recaptcha-response']);
                $response = json_decode($response['body']);
                ...
                if ($response->success) {
                    ...
                    return new WP_Error('wpcaptcha_recaptchav3_failed', __("<strong>ERROR</strong>: reCAPTHCA verification failed.<br /><br />Please try again.", 'advanced-google-recaptcha'));
                }
            }
        } else if ($options['captcha'] == 'builtin') {
            if (isset($_POST['wpcaptcha_captcha']) && $_POST['wpcaptcha_captcha'] === $_COOKIE['wpcaptcha_captcha']) {
                return true;
            } else {
    ...
    static function check_woo_register_form($username, $email, $errors)
    {
        if (wp_doing_ajax()) {
            return;
        }
    ...
    static function captcha_fields()
    {
        ...
        function wpcaptcha_captcha(){
            ...
            echo '<img class="wpcaptcha-captcha-img" style="vertical-align: text-top;" src="' . esc_url(WPCAPTCHA_PLUGIN_URL) . '/libs/captcha.php?wpcaptcha-generate-image=true&color=' . esc_attr(urlencode('#FFFFFF')) . '&noise=1&rnd=' . (int)rand(0, 10000) . '" alt="Captcha" />';
            echo '<input class="input" type="text" size="3" name="wpcaptcha_captcha" id="wpcaptcha_captcha" />';
            echo '</label></p><br />';
        }
    }
